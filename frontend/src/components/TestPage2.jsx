...
export default class TestPage extends React.Component {
	...
	componentDidMount() {
		...
		var filter1 = this.getFilter(queryString.parse(this.props.location.search).subject)
		this.setState({filter1 : filter1})
		console.log("page: ", queryString.parse(this.props.location.search).page)
		console.log("subject: ", queryString.parse(this.props.location.search)["subject"])
	}

	getFilter (filterArray) {
		let json = {"or" : []};
	...
	combineFilters(filter1, orderByArray, searchText) {
		return { "search":searchText, "filters": [ { "and":[filter1]}]};
	}

	generateBackendQuery(filter1, orderByArray, searchText) {
		return { "search":searchText, "filters": [ { "and":[filter1]}]};
	}
	...
	render() {
		...
		//let defaultValue = [{value : "Animals", label : "Animals"},{value : "Commerce", label : "Commerce"}];
		//let subjectArray = queryString.parse(this.props.location.search).subject
		//console.log(subjectArray)
		//let defaultValue = [{value: subjectArray[0], label : subjectArray[0]},{value: subjectArray[1], label : subjectArray[1]}];
						...
						<Row>
							<Col xs="6" sm={{ size: '8', offset: '1' }}>
								<Filter 
									...
									filterCallBack = {this.filterCallBack1} 
