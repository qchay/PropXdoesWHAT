...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \code{`cdotgov\_url` VARCHAR(256),}					-- https://www.congress.gov/bill/115th-congress/house-bill/5515
		\item \code{`govtrack\_url` VARCHAR(256),}		-- https://www.govtrack.us/congress/bills/115/hr5515
	
		\item \code{`introduced` DATE,	}				-- YYYY-MM-DD
		\item \code{`last\_vote` DATE,}					-- YYYY-MM-DD
		\item \code{`house\_pass` DATE,}					-- YYYY-MM-DD
		\item \code{`senate\_pass` DATE,	}				-- YYYY-MM-DD
		\item \code{`enacted` DATE,	}					-- YYYY-MM-DD
		\item \code{`vetoed` DATE,	}					-- YYYY-MM-DD
	
	\item \code{`desc` TEXT,}						-- propub summary
